...
import { KeyRound, Link2, Link2Off, AlertTriangle, Copy, Check as CheckIcon } from 'lucide-react'
...
export function ConfigPage() {
  ...
  async function handleToggleActive(d: CustomDomain) {
    try {
      ...
      setDomains(prev => prev.map(x => x.id === d.id ? { ...x, verified: !d.verified } : x))
    } catch (e: any) {
      alert(e.message)
    }
  }
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <DarkCard title="Dominio personalizado" icon={<Globe size={16} />}>
        <p className="text-sm text-slate-400">
          Conecta tu propio dominio (ej. <span className="font-mono text-slate-300">reservas.mihotel.com</span>)
          para que los clientes accedan a tu web de reservas con tu marca. El dominio activo aparece
          como principal en la plataforma.
        </p>
        ...
        {domains.length > 0 && (
          <div className="overflow-hidden rounded-xl border border-slate-700">
            {domains.map((d) => (
              <div key={d.id} className="flex items-center justify-between gap-3 border-b border-slate-700 px-4 py-3 last:border-b-0">
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <button
                    ...
                    className="rounded-xl p-1.5 text-slate-500 hover:bg-red-500/10 hover:text-red-400 transition"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-slate-700 bg-slate-900 px-4 py-2.5 text-sm font-mono text-slate-100 placeholder-slate-600 focus:border-brand-400 focus:outline-none"
          />
          <button
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="shrink-0"
                >
                  {domainCopied ? <CheckIcon size={12} className="text-emerald-400" /> : <Copy size={12} className="text-slate-500 hover:text-slate-300" />}
                </button>
              </div>
            </div>
          </div>
          <p className="text-xs text-slate-500">
            Una vez configurado el DNS y verificado que el dominio resuelve correctamente, actívalo con el botón <strong className="text-slate-400">Activar</strong>.
            La propagación DNS puede tardar hasta 24h.
          </p>
        </div>
      </DarkCard>
