        ...
        <div className="px-6 py-5 border-b border-slate-200 bg-white">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Gestión centralizada de contactos</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={loadData} className="p-2 rounded-lg hover:bg-slate-100 text-slate-400 transition-colors">
                <RefreshCw size={15} />
              </button>
              <button onClick={() => navigate('/admin/reservas/nueva')}
                className="flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-700 transition-all">
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-slate-50 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-transparent"
            />
          </div>
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-50 ${
        isSelected ? 'bg-slate-100 border-l-2 border-brand-600' : ''
      }`}
    >
      {/* Avatar */}
      <div className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${
        c.reservas.length > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
      }`}>
...
function ContactDetail({ contacto: c, onClose, onRefresh }: {
  ...
  return (
    <div className="relative flex-1 flex flex-col border-l border-slate-200 bg-white overflow-hidden">

      {/* Modal editar */}
      {editOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl w-96 p-6">
            <h3 className="font-bold text-slate-900 mb-4">Editar cliente</h3>
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                />
              </div>
              <div>
              ...
              <p className="text-[11px] text-slate-400">Se actualizará en todas las reservas y consultas de este email.</p>
            </div>
            <div className="flex gap-2 mt-5">
              ...
              <button
                ...
                disabled={saving}
                className="flex-1 py-2 rounded-lg bg-brand-600 text-white text-sm hover:bg-brand-700 transition-colors disabled:opacity-50"
      ...
      {deleteConfirm && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl w-96 p-6">
            <h3 className="font-bold text-slate-900 mb-2">¿Borrar cliente?</h3>
            <p className="text-sm text-slate-500 mb-1">
              Se eliminarán <strong>todas las reservas y consultas</strong> asociadas al email:
            </p>
            <p className="text-sm font-semibold text-slate-800 mb-4">{c.email}</p>
      ...
      <div className="px-6 py-4 border-b border-slate-200 flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className={`h-12 w-12 rounded-full flex items-center justify-center text-base font-bold ${
            c.reservas.length > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
          }`}>
            {c.nombre[0]?.toUpperCase() ?? '?'}
          </div>
          <div>
            <h2 className="font-bold text-slate-900">{c.nombre}</h2>
      ...
      <div className="grid grid-cols-3 gap-px bg-slate-100 border-b border-slate-200">
        <div className="bg-white px-4 py-3 text-center">
          ...
          <p className="font-bold text-slate-900 text-lg">{c.consultas.length}</p>
        </div>
        <div className="bg-white px-4 py-3 text-center">
        ...
        <div className="bg-white px-4 py-3 text-center">
          ...
          <p className="font-bold text-slate-900 text-lg">{totalGastado.toFixed(0)}€</p>
        </div>
      </div>
        ...
        {([['info', 'Información'], ['reservas', 'Reservas'], ['consultas', 'Consultas']] as const).map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)}
            className={`flex-1 py-3 text-xs font-semibold transition-colors border-b-2 -mb-px ${
              tab === k ? 'border-brand-600 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
            }`}
...
function ReservaCard({ reserva: r }: { key?: React.Key; reserva: Reserva }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 overflow-hidden">
      {/* Cabecera */}
      <button onClick={loadDetail} className="w-full flex items-start justify-between p-4 hover:bg-slate-50 transition-colors text-left">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold text-slate-800">{r.codigo}</span>
            <ReservaEstadoBadge estado={r.estado} />
          ...
          <p className="text-sm font-semibold text-slate-900">
            ...
            <span className="text-slate-400 font-normal ml-2">· {nights} noches</span>
          </p>
        ...
        <div className="text-right shrink-0 ml-3">
          <p className="text-base font-bold text-slate-900">{Number(r.total).toFixed(0)}€</p>
          <p className="text-[10px] text-slate-400">pagado: {Number(r.importe_pagado).toFixed(0)}€</p>
          {loadingDetail
            ? <Loader2 size={14} className="animate-spin text-slate-400 mt-1 ml-auto" />
            : <ChevronRight size={14} className={`text-slate-300 mt-1 ml-auto transition-transform ${open ? 'rotate-90' : ''}`} />
          }
        </div>
      </button>
      ...
      {open && (
        <div className="border-t border-slate-100 bg-slate-50/50 p-4 space-y-4">
          ...
          <div className="rounded-lg bg-white border border-slate-200 p-3 space-y-1.5 text-xs">
            <p className="font-semibold text-slate-700 mb-2">Desglose del importe</p>
          ...
          {huespedes.length > 0 && (
            <div>
              ...
              <div className="space-y-1.5">
                {huespedes.map(h => (
                  <div key={h.id} className={`rounded-lg border p-2.5 text-xs flex items-start justify-between ${h.completado ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 bg-white'}`}>
                    <div>
                      <p className="font-semibold text-slate-800">
                        {h.completado ? `${h.nombre} ${h.primer_apellido}` : 'Pendiente de registro'}
                        {h.titular && <span className="ml-1.5 text-[9px] bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded font-bold uppercase">Titular</span>}
                      </p>
                      {h.completado && (
                        <p className="text-slate-400 mt-0.5">{h.tipo_documento}: {h.numero_documento} · {h.nacionalidad}</p>
                      )}
                    </div>
                    {h.completado
          ...
          {huespedes.length === 0 && (
            <div className="rounded-lg border border-slate-200 p-3 text-xs text-slate-400 text-center">
          ...
          {facturas.length > 0 && (
            <div>
              ...
              {facturas.map(f => (
                <div key={f.id} className="rounded-lg border border-slate-200 bg-white p-2.5 flex items-center justify-between text-xs">
                  <div>
                    <p className="font-semibold text-slate-800">{f.numero}</p>
                    <p className="text-slate-400">{format(parseISO(f.fecha_emision), "d MMM yyyy", { locale: es })} · {Number(f.total).toFixed(2)}€</p>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          {facturas.length === 0 && (
            <div className="rounded-lg border border-dashed border-slate-200 p-3 text-xs text-slate-400 text-center">
              Sin factura emitida
            </div>
          )}
        </div>
...
function ConsultaCard({ consulta: c, onRefresh }: { key?: React.Key; consulta: Consulta; onRefresh: () => void }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${c.estado === 'NUEVA' ? 'border-amber-200' : 'border-slate-200'}`}>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-start justify-between p-3 text-left hover:bg-slate-50 transition-colors">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <ConsultaEstadoBadge estado={c.estado} />
            <span className="text-[10px] text-slate-400">{format(parseISO(c.created_at), "d MMM yyyy HH:mm", { locale: es })}</span>
          </div>
          <p className="text-sm font-semibold text-slate-800 line-clamp-1">{c.asunto || 'Consulta general'}</p>
          {!open && <p className="text-xs text-slate-400 line-clamp-1 mt-0.5">{c.mensaje}</p>}
        </div>
        <ChevronRight size={14} className={`text-slate-300 shrink-0 mt-1 transition-transform ${open ? 'rotate-90' : ''}`} />
      </button>

      {open && (
        <div className="border-t border-slate-100 p-3 space-y-3">
          <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed bg-slate-50 rounded-lg p-3">{c.mensaje}</p>

          {/* Notas admin */}
          <div>
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Notas internas</label>
            <textarea
              value={notas}
              onChange={e => setNotas(e.target.value)}
              rows={2}
              placeholder="Añade notas sobre esta consulta..."
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand-500/20 resize-none"
            />
            <button
              onClick={saveNotas}
              disabled={saving}
              className="mt-1.5 px-3 py-1.5 bg-brand-600 text-white text-xs rounded-lg hover:bg-brand-700 transition-colors disabled:opacity-50"
            >
          ...
          {/* Cambiar estado */}
          <div>
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Cambiar estado</label>
              ...
              {(['NUEVA', 'VISTA', 'RESPONDIDA', 'ARCHIVADA'] as EstadoConsulta[]).map(e => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-colors ${
                    c.estado === e ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                  }`}
                >
...
function Stat({ label, value, color }: { label: string; value: number | string; color?: string }) {
  const cls = color === 'amber' ? 'text-amber-600' : color === 'emerald' ? 'text-emerald-600' : color === 'blue' ? 'text-blue-600' : 'text-slate-900'
  return (
...
function PriceRow({ label, value, className }: { label: string; value: string; className?: string }) {
  return (
    <div className={`flex justify-between text-slate-500 ${className ?? ''}`}>
...
function ReservaEstadoBadge({ estado }: { estado: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${map[estado] ?? 'bg-slate-100 text-slate-500'}`}>{labels[estado] ?? estado}</span>
}
...
function ConsultaEstadoBadge({ estado }: { estado: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${map[estado] ?? 'bg-slate-100 text-slate-500'}`}>{estado}</span>
}
