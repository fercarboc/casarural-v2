  ...
  return (
    <div>
      ...
      <input
        ...
        required={required}
        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-900 focus:border-brand-400 focus:outline-none focus:bg-white transition-colors"
      />
    </div>
  )
}
...
function SuccessScreen({ reserva, onNew }: { reserva: CreatedReserva; onNew: () => void }) {
  ...
  return (
    <div className="max-w-lg mx-auto py-16 text-center space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-900">¡Reserva creada!</h1>
        <p className="mt-2 text-slate-500">
          <span className="font-mono font-bold text-slate-700">{reserva.codigo}</span>
          {' · '}{reserva.nombre} {reserva.apellidos}
        </p>
        <p className="mt-1 text-lg font-bold text-slate-900">{reserva.total.toFixed(2)} €</p>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white p-5 text-left space-y-3">
        <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Acciones</p>

        {/* Solicitud de pago Stripe */}
        {reserva.metodo_pago_previsto === 'STRIPE' && (
          <button onClick={() => setShowPago(true)}
            className="w-full flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-all">
        ...
        <button onClick={sendCheckin} disabled={sendingCheckin || checkinSent}
          className="w-full flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 transition-all">
          {checkinSent
            ? <><Check size={16} className="text-emerald-600 shrink-0" /> Email check-in enviado</>
            : sendingCheckin
              ? <><Loader2 size={16} className="animate-spin text-slate-400 shrink-0" /> Enviando...</>
              : <><Send size={16} className="text-slate-400 shrink-0" /> Enviar enlace de check-in</>}
        </button>

        {/* Ver reserva */}
        <button onClick={() => navigate(`/admin/reservas/${reserva.id}`)}
          className="w-full flex items-center gap-3 rounded-xl bg-brand-600 px-4 py-3 text-sm font-bold text-white hover:bg-brand-700 transition-all">
          <Eye size={16} className="shrink-0" /> Ver reserva
        </button>

        {/* Crear otra */}
        <button onClick={onNew}
          className="w-full flex items-center gap-3 rounded-xl border border-dashed border-slate-300 px-4 py-3 text-sm font-semibold text-slate-500 hover:border-slate-400 hover:text-slate-700 transition-all">
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <header className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Nueva reserva manual</h1>
          <p className="text-sm text-slate-500">Confirmada directamente por el administrador</p>
        </div>
      </header>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4">
          <h2 className="text-sm font-bold text-slate-700">1 · Datos del cliente (titular)</h2>
        </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4">
          <h2 className="text-sm font-bold text-slate-700">2 · Datos de la estancia</h2>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Fecha entrada *</label>
              <input type="date" value={fecha_entrada} min={today}
                onChange={e => setFechaEntrada(e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-900 focus:border-brand-400 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Fecha salida *</label>
              <input type="date" value={fecha_salida} min={fecha_entrada || today}
                onChange={e => setFechaSalida(e.target.value)}
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-900 focus:border-brand-400 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Nº huéspedes (1-11) *</label>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Menores (informativo)</label>
          ...
          {/* Tarifa */}
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Tarifa *</label>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button key={opt.value} type="button" onClick={() => setTarifa(opt.value as any)}
                  className={`rounded-xl border-2 p-4 text-left transition-all ${tarifa === opt.value ? 'border-brand-600 bg-slate-50 ring-2 ring-slate-900/10' : 'border-slate-200 hover:border-slate-300'}`}>
                  <div className="flex justify-between items-center">
                    <p className="text-sm font-bold text-slate-900">{opt.label}</p>
                    {tarifa === opt.value && <Check size={14} className="text-slate-900" />}
                  </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
          <h2 className="text-sm font-bold text-slate-700">3 · Desglose de precio</h2>
          {calculating && (
        ...
        <div className="p-6">
          {!fecha_entrada || !fecha_salida || fecha_salida <= fecha_entrada
            ? <p className="text-sm text-slate-400 text-center py-4">Introduce las fechas para ver el precio</p>
            : priceError
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-xl border border-slate-200 overflow-hidden text-sm">
                      ...
                      <div className="flex justify-between px-4 py-2.5 border-t border-slate-100">
                        <span className="text-slate-500">{priceResult.nights} noches × {priceResult.precio_noche} €/noche</span>
                        <span className="font-medium">{priceResult.importe_alojamiento.toFixed(2)} €</span>
                      </div>
                      {priceResult.importe_extra > 0 && (
                        <div className="flex justify-between px-4 py-2.5 border-t border-slate-100">
                          <span className="text-slate-500">Suplemento huésped extra ({priceResult.extra_guests} × {priceResult.extra_huesped} € × {priceResult.nights} noches)</span>
                          <span className="font-medium">{priceResult.importe_extra.toFixed(2)} €</span>
                        </div>
                      )}
                      <div className="flex justify-between px-4 py-2.5 border-t border-slate-100">
                        <span className="text-slate-500">Gastos de limpieza</span>
                        <span className="font-medium">{priceResult.limpieza.toFixed(2)} €</span>
                      </div>
                      {priceResult.descuento > 0 && (
                        <div className="flex justify-between px-4 py-2.5 border-t border-slate-100 text-emerald-700">
                          <span>Descuento no reembolsable (−10%)</span>
                          <span className="font-medium">−{priceResult.descuento.toFixed(2)} €</span>
                        </div>
                      )}
                      <div className="flex justify-between px-4 py-3 border-t-2 border-slate-200 font-bold text-slate-700">
                    ...
                    {/* Total final */}
                    <div className={`rounded-xl p-5 border ${descuentoNum > 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
                      <div className="flex justify-between items-baseline">
                        <span className="text-sm font-bold text-slate-700 uppercase tracking-wider">Total final</span>
                        <span className="text-3xl font-bold text-slate-900">{totalFinal.toFixed(2)} €</span>
                      </div>
                      {descuentoNum > 0 && (
                      ...
                      {tarifa === 'FLEXIBLE' && importeSenalFinal != null && (
                        <div className="flex justify-between mt-2 pt-2 border-t border-slate-200 text-sm">
                          <span className="text-slate-500">Señal (50%)</span>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4">
          <h2 className="text-sm font-bold text-slate-700">4 · Estado del pago</h2>
        </div>
        <div className="p-6 grid grid-cols-2 gap-6">
          {/* Estado */}
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Estado del pago</label>
            <div className="space-y-2">
              {[
                ...
                <label key={opt.value}
                  className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${estado_pago === opt.value ? 'border-brand-600 bg-slate-50' : 'border-slate-200 hover:border-slate-300'}`}>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Método previsto</label>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4">
          <h2 className="text-sm font-bold text-slate-700">5 · Notas</h2>
        </div>
        <div className="p-6 grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Notas internas (admin)</label>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Notas para el cliente (email)</label>
      ...
      <div className="flex gap-3">
        <button onClick={() => navigate('/admin/reservas')}
          className="rounded-xl border border-slate-200 px-6 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
          Cancelar
        </button>
        <button onClick={handleSave} disabled={!canSave || saving}
          className="flex-1 rounded-xl bg-brand-600 px-6 py-3 text-sm font-bold text-white hover:bg-brand-700 disabled:opacity-40 flex items-center justify-center gap-2 transition-all">
          {saving
