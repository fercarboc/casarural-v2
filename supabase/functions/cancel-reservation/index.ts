// supabase/functions/cancel-reservation/index.ts  [v2]
...
serve(async (req) => {
  ...
  try {
    ...
    const [{ data: reserva }, { data: pagos }] = await Promise.all([
      ...
      supabase.from('pagos').select('importe, estado, tipo').eq('reserva_id', reservaId),
    ]);
    ...
    // Eliminar bloqueos asociados (liberar disponibilidad)
    await supabase
      .from('bloqueos')
      .delete()
      .eq('origen', 'RESERVA')
    ...
    // Nota: reserva_unidades se elimina en cascada (ON DELETE CASCADE en el esquema)
    ...
    if (importeReembolso > 0 && reserva.stripe_payment_intent) {
      try {
        ...
        // Registrar retención si aplica (porcentaje no reembolsado)
        if (porcentajeReembolso < 1) {
          const totalPagado = (pagos ?? [])
            .filter((p: any) => p.estado === 'COMPLETED' && p.tipo !== 'REEMBOLSO')
            .reduce((sum: number, p: any) => sum + Number(p.importe), 0);
          ...
          if (retencion > 0) {
            await supabase.from('retenciones_cancelacion').insert({
              ...
              concepto:    `Retención por cancelación (política: ${porcentajeReembolso * 100}% reembolso)`,
            });
          }
        }
      } catch (stripeErr) {
    ...
    return Response.json({
      ...
      porcentaje_reembolso: porcentajeReembolso * 100,
    }, { headers: corsHeaders });
